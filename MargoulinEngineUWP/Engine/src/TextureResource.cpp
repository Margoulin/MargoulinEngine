...
auto	TextureResource::InitializeD3D11Datas(ID3D11Device* device) -> void
{
	if (!imageData)
		return;
...
auto	TextureResource::CreateTextureRenderData() const -> TextureRenderData
{
	TextureRenderData ret;
...
auto	TextureResource::CreateTextureRenderData(Vector4F const& newRect) const -> TextureRenderData
{
	TextureRenderData ret;
	ret.samplerState = samplerState.Get();
	ret.shaderView = shaderView.Get();
